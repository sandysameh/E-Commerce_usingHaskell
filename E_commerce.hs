...
helpconc x [] = []
helpconc x ((item,list):ys) = if x==item then list ++ helpconc x ys else helpconc x ys
freqListCart user []=[]
...
freqListUsers user = test (khodElList (makeOne((purchasesIntersection (ya5odGambElUser user (getAllUsersStats purchasesHistory)) (removeItem user (getAllUsersStats purchasesHistory))))))

removeItem _ [] = []
removeItem user ((u,list):xs) | user == u  = xs
                    | otherwise = (u,list) : removeItem user xs
